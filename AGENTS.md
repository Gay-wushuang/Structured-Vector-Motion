...
38. `examples/001-head-basic.svm.json` — current example Document.
39. `tests/test_golden_a.py` — executable expectations.
...
- Numeric storage does not imply animatability in Motion v0.2; targets must be
  declared explicitly by the active Operation Definition. Motion v0.1 retains
  its recorded legacy numeric-target rule for compatibility.
...
16. Anchored Regeneration and Golden O derive exact Proposal impact from the
...
28. Explicit Temporal Identity Promotion and Golden R1 promote only selected
...
30. Golden S1 explicitly binds one stable Temporal Identity to one existing
