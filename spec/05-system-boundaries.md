...
Path-to-planar conversion is implemented as the explicit `PathToPolygon`
Operation in `12-path-to-planar-geometry.md`. `BooleanGeometry` and its Backend
must never flatten `path_data` implicitly.
