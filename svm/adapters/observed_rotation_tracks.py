...
class ObservedRotationTracksAdapter:
    ...
    def propose(self, request: AdapterRequest, artifacts: ArtifactRepository) -> Proposal:
        ...
        changes: list[Any] = [
            CreateGroupTransformTrackChange(
                track_id, group_id, "rotation_degrees", ticks_per_second, "linear"
            )
        ]
        changes.extend(
        ...
        generator = GeneratorProvenance(
            ...
            "svm-verified-observed-rotation-authoring",
            POLICY_IDENTITY,
            {
        ...
        return Proposal(
            ...
            transaction=Transaction(
                f"transaction:observed-rotation-track:{digest}",
                tuple(changes),
                "Author verified observed rotation Track",
            ),
...
def verify_observed_rotation_track_source(
    ...
    samples = _absolute_samples(
        payload["intervals"], change.group.get("transform", {}).get("rotation_degrees")
    )
    track_id, keyframes = _track_definition(
        change.binding["id"],
        snapshot.artifact_id,
...
def _absolute_samples(
    ...
    for index, interval in enumerate(intervals):
        ...
        if (
            ...
            raise ObservedRotationTracksError(
                "Observed rotation intervals must form one supported ordered contiguous chain"
            )
        if index == 0:
            samples.append((source_tick, value, None))
        value = value + cast(int | float, delta)
...
def _track_definition(binding_id, artifact_id, group_id, samples, ticks_per_second):
    ...
    keyframes = tuple(
        (
            "keyframe:observed-rotation:"
            + hashlib.sha256(canonical_bytes({"track_id": track_id, "tick": tick})).hexdigest(),
            tick,
            value,
            delta,
        )
        for tick, value, delta in samples
    )
    return track_id, keyframes


def _document_track(
    group_id, track_id, keyframes, binding_id, evidence_artifact_id, source_revision_id
):
    return {
        "id": track_id,
        "target": {"group": group_id, "property": "rotation_degrees"},
        "value_type": "number",
...
def _expected_animation(animation_before, authored_track, ticks_per_second):
    ...
    if (
        ...
        and animation["timebase"].get("ticks_per_second") != ticks_per_second
    ):
        raise ObservedRotationTracksError(
